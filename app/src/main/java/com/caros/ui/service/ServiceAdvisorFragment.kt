    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.serviceItems.collectLatest { items ->
                ...
                val urgentCount = items.count { it.status == ServiceUrgency.URGENT }
...
class ServiceItemAdapter : RecyclerView.Adapter<ServiceItemAdapter.VH>() {
    private val items = mutableListOf<ServiceItem>()
    fun submitList(list: List<ServiceItem>) { items.clear(); items.addAll(list); notifyDataSetChanged() }
