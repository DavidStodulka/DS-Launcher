...
KROK A — PATCHED BOOT:
  ...
  výsledek přenést do PC a PŘEJMENOVAT přesně na boot_patched.img (pevný název =
  konec problémů s hledáním souboru). (Zdůrazni: NE Direct Install.)
...
KROK C — SESTAV ROOT USB (script-only, bez full reflashe):
  Do KOŘENE:
    ...
    - config.txt          (z Mekede\)
    - boot_patched.img    (přejmenovaný patched boot z kroku A)
  ...
  obsahem (v4: pevný název + kontrola velikosti + plný výpis USB do logu;
  ulož UTF-8 BEZ BOM, LF, žádné \r — jinak dd selže):
  ------------------------------------------------------------
  #!/system/bin/sh
